...
public class PlayerStats : MonoBehaviour, IDamageable
{
    ...
    // Fall damage method that doesn't trigger knockback or other effects
    public void TakeFallDamage(int damage)
    {
        // Don't take damage during invincibility frames
        if (isInvincible) 
        {
            Debug.Log("Player is invincible - fall damage ignored");
            return;
        }
        
        currentHealth -= damage;
        Debug.Log($"Player took {damage} fall damage. Health: {currentHealth}/{maxHealth}");
        
        // Invoke the event for UI updates
        OnHealthChanged?.Invoke(currentHealth, maxHealth);
        
        // Apply screen flash effect (but no knockback)
        FlashScreen();
        
        // Flash and brief invincibility (but no knockback)
        StartInvincibilityFrames();
