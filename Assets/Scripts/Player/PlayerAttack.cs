...
public class PlayerAttack : MonoBehaviour
{
    ...
    // New persistent hitbox system
    private GameObject attackHitbox;
    private CircleCollider2D attackCollider;
    private HashSet<GameObject> hitEnemies = new HashSet<GameObject>();
    private float attackStartTime = 0f;
    private bool isPersistentAttackActive = false;
    ...
    {
        // Create a child GameObject for the attack hitbox
        ...
        // Add a CircleCollider2D as trigger
        attackCollider = attackHitbox.AddComponent<CircleCollider2D>();
        attackCollider.isTrigger = true;
        attackCollider.radius = attackRange;
        
        // Add a script to handle trigger events
        AttackHitboxTrigger triggerScript = attackHitbox.AddComponent<AttackHitboxTrigger>();
        triggerScript.playerAttack = this;
    ...
    private void Update()
    {
        ...
        // Update persistent attack hitbox
        if (isPersistentAttackActive)
        {
            UpdatePersistentAttack();
        }
    }

    private void UpdatePersistentAttack()
    {
        ...
        // Update the attack hitbox position and rotation to follow the player
        if (attackHitbox != null)
        {
            attackHitbox.transform.position = transform.position;
            
            // Update the hitbox rotation to match the current attack angle
            float currentAngle = attackPivot.rotation.eulerAngles.z;
            attackHitbox.transform.rotation = Quaternion.Euler(0, 0, currentAngle);
        }
    }

    private void StartPersistentAttack()
    {
        isPersistentAttackActive = true;
    ...
    private void EndPersistentAttack()
    {
        isPersistentAttackActive = false;
        hitEnemies.Clear();
    ...
    // Called by AttackHitboxTrigger when an enemy enters the attack area
    public void OnEnemyEnterAttackArea(GameObject enemy)
    {
        if (!isPersistentAttackActive || hitEnemies.Contains(enemy))
        {
            return;
        }
        ...
        if (enemyHealth != null)
        {
            ...
            // Check if this is a dash attack (deal more damage)
            int damage = isDashing ? attackDamage * 2 : attackDamage;
            enemyHealth.TakeDamage(damage, 0f, false, knockbackDirection);
        }
        else
        {
    ...
    private void HandleMouseInput()
    {
        ...
        if (IsPointerOverUI())
        {
            return; // Don't process attack input if over UI
        }
    ...
    private void StartDash(float distance, float attackSweepAngle)
    {
        ...
        // Start persistent attack hitbox for dash
        StartPersistentAttack();
    ...
    private void EndDash()
    {
        ...
        // End persistent attack hitbox
        EndPersistentAttack();
    }
    
    private void CheckDashHits()
    {
        // This method is now handled by the persistent hitbox system
    ...
    private void StartAttack(float attackSweepAngle)
    {
        ...
        // Start persistent attack hitbox
        StartPersistentAttack();
    }
    ...
    {
        return isAttacking || isDashing || isPersistentAttackActive;
    }
...
// Helper class to handle trigger events for the attack hitbox
public class AttackHitboxTrigger : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        ...
        if (other.CompareTag("Enemy") || other.GetComponent<EnemyHealth>() != null || other.GetComponent<IDamageable>() != null)
        {
            playerAttack.OnEnemyEnterAttackArea(other.gameObject);
        }
    }
} 