...
/// <summary>
/// Handles the second cracked wall puzzle in the dungeon (not in boss room).
...
/// </summary>
public class CrackedWall2 : MonoBehaviour, IPointerClickHandler
{
    ...
    public SpellBeamGridController spellGridController;
    ...
    void Start()
    {
        ...
        if (player == null)
        {
            ...
            if (player == null)
            {
                Debug.LogWarning("CrackedWall2: No player found with 'Player' tag. Please assign the player reference manually.");
            }
        }
        ...
        {
            Debug.LogError("CrackedWall2: Back button is not assigned! Please assign it in the inspector.");
        }
        ...
        {
            Debug.LogError("CrackedWall2: Puzzle UI panel is not assigned! Please assign it in the inspector.");
        }
        ...
        {
            Debug.LogWarning("CrackedWall2: Background image is not assigned. The puzzle will work without it.");
        }
        ...
        {
            Debug.LogError("CrackedWall2: Spell grid controller is not assigned! Please assign it in the inspector.");
        }
        ...
        {
            Debug.LogWarning("CrackedWall2: No SpriteRenderer or Image component found. Visual changes may not work.");
        }
    ...
    private void SetupInteractionComponents()
    {
        ...
        {
            Debug.Log("CrackedWall2: Adding solid BoxCollider2D for blocking.");
        ...
        if (rb == null)
        {
            Debug.Log("CrackedWall2: Adding Rigidbody2D for interaction.");
        ...
        {
            Debug.Log("CrackedWall2: Adding trigger BoxCollider2D for interaction detection.");
    ...
    private void SetupMouseInputComponents()
    {
        ...
        if (canvas != null)
        {
            Debug.Log("CrackedWall2: Detected as UI element, ensuring GraphicRaycaster setup.");
        ...
        {
            Debug.Log("CrackedWall2: Setting up for world-space mouse input.");
    ...
    private void ValidateUIReferences()
    {
        ...
        if (Application.isPlaying)
        {
            ...
            if (puzzleUIPanel == null)
            {
                puzzleUIPanel = GameObject.Find("PuzzleUIPanel2");
                if (puzzleUIPanel != null)
                {
                    Debug.Log("CrackedWall2: Found PuzzleUIPanel in scene automatically.");
                }
            }
            
            if (backButton == null)
            {
                Button foundButton = GameObject.Find("BackButton2")?.GetComponent<Button>();
                if (foundButton != null)
                {
                    backButton = foundButton;
                    Debug.Log("CrackedWall2: Found BackButton in scene automatically.");
                }
            }
            
            if (spellGridController == null)
            {
                spellGridController = GameObject.Find("SpellGrid2")?.GetComponent<SpellBeamGridController>();
                if (spellGridController != null)
                {
                    Debug.Log("CrackedWall2: Found SpellGridDragController in scene automatically.");
                }
            }
            
            if (backgroundImage == null)
            {
                backgroundImage = GameObject.Find("PuzzleBackground2")?.GetComponent<Image>();
                if (backgroundImage != null)
                {
                    Debug.Log("CrackedWall2: Found background image in scene automatically.");
                }
            }
        }
    }
    ...
    {
        Debug.Log("CrackedWall2: OnPointerClick triggered!");
        TryInteract();
    }
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        ...
        {
            Debug.Log("CrackedWall2: Player entered trigger area!");
    ...
    void OnTriggerExit2D(Collider2D other)
    {
        if (other.CompareTag("Player"))
        {
            Debug.Log("CrackedWall2: Player exited trigger area!");
            playerInRange = false;
        }
    }
    ...
    private void CheckSpawnRange()
    {
        ...
        {
            Debug.Log($"CrackedWall2: Player entered spawn range ({distance:F1} <= {spawnDistance})");
        }
        else if (!playerInSpawnRange && wasInSpawnRange)
        {
            Debug.Log($"CrackedWall2: Player exited spawn range ({distance:F1} > {spawnDistance})");
        }
    }
    ...
    protected virtual void TryInteract()
    {
        ...
        if (IsPlayerNearby())
        {
            Debug.Log("CrackedWall2: Opening puzzle!");
            OpenPuzzle();
        }
        else
        {
    ...
    {
        Debug.Log("CrackedWall2: Manual interaction triggered!");
        TryInteract();
    }
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.E))
        {
            if (IsPlayerNearby())
            {
                Debug.Log("CrackedWall2: E key pressed - testing interaction!");
                Interact();
            }
        }
    ...
    private void StartSpawnIndicator(Vector3 spawnPosition)
    {
        ...
        if (isPuzzleActive || SpawnIndicator.AreSpawnIndicatorsPaused())
        {
            Debug.Log("CrackedWall2: Cannot start spawn indicator - puzzle active or indicators paused");
            return;
        }
        ...
        Debug.Log($"CrackedWall2: Started spawn indicator traveling from wall to position {spawnPosition}");
    }
    ...
    private System.Collections.IEnumerator SpawnAfterIndicator()
    {
        ...
        else
        {
            ...
            isSpawnIndicatorActive = false;
            pendingSpawnPosition = Vector3.zero;
            Debug.Log("CrackedWall2: Spawn cancelled due to pause or indicator not active");
        }
    }
    ...
    protected virtual void SpawnEnemyAtPosition(Vector3 spawnPosition)
    {
        if (enemyPrefab == null)
        {
            Debug.LogWarning("CrackedWall2: No enemy prefab assigned!");
            return;
        }
        ...
        Debug.Log($"CrackedWall2: Spawned enemy {enemiesSpawned}/{maxEnemies} at position {spawnPosition}");
    ...
    private void CancelPendingSpawns()
    {
        if (isSpawnIndicatorActive)
        {
            ...
            Debug.Log("CrackedWall2: Cancelled pending enemy spawn due to puzzle pause");
        }
    }
    ...
    private void SpawnEnemy()
    {
        ...
        {
            Debug.LogWarning("CrackedWall2: Could not find valid spawn position for enemy!");
        }
    }
    ...
    protected virtual Vector3 FindValidSpawnPosition()
    {
        ...
        {
            Debug.LogError("CrackedWall2: No DungeonGenerator found in scene!");
            return Vector3.zero;
        }
        ...
        for (int attempt = 0; attempt < 20; attempt++)
        {
                ...
                gridY >= 0 && gridY < dungeonGenerator.dungeonHeight)
            {
                ...
                {
                    Debug.Log($"CrackedWall2: Found valid spawn position at ({gridX}, {gridY}) - TileType: {tileType}");
                    return new Vector3(gridX, gridY, 0);
                }
            }
        }
        
        Debug.LogWarning("CrackedWall2: Failed to find valid spawn position after 20 attempts!");
        return Vector3.zero;
    }
    ...
    private void CheckMouseClick()
    {
        ...
        {
            Debug.Log("CrackedWall2: Mouse click detected via raycast!");
            TryInteract();
        }
    }
    ...
    private bool IsPlayerNearby()
    {
        ...
        if (inDistance)
        {
            Debug.Log($"CrackedWall2: Player is within distance ({distance:F2} <= {interactionDistance})");
        }
        else
        {
            Debug.Log($"CrackedWall2: Player is too far ({distance:F2} > {interactionDistance})");
        }
    ...
    private void ClosePuzzle()
    {
        if (puzzleUIPanel != null)
        {
            ...
            // Clear all spell beams when puzzle is closed
            if (spellGridController != null)
            {
                spellGridController.OnPuzzleClosed();
            }
    ...
    {
        if (pattern == correctSpellPattern)
        {
            Debug.Log($"Correct pattern entered: {pattern}. Showing completed spell for 2 seconds...");
            StartCoroutine(SolvePuzzleWithDelay());
        }
        else
        {
    ...
    {
        // Note: isPuzzleSolved and isPuzzleActive are already set in SolvePuzzleWithDelay()
        ...
        // Clear all spell beams when puzzle is solved (after the delay)
        if (spellGridController != null)
        {
            spellGridController.OnPuzzleSolved();
        }
        ...
        // Notify HintManager to destroy hint2miniimage when puzzle2 is solved
        if (HintManager.Instance != null)
        {
            HintManager.Instance.DestroyHintMiniImage("hint2");
            Debug.Log("CrackedWall2: Notified HintManager to destroy hint2miniimage");
        }
    }
    
    /// <summary>
    /// Coroutine that delays puzzle completion to show the completed spell pattern
    /// </summary>
    private System.Collections.IEnumerator SolvePuzzleWithDelay()
    {
        // Mark puzzle as solved immediately to prevent further input
        isPuzzleSolved = true;
        isPuzzleActive = false;
        
        // Visual feedback for solving the puzzle
        Debug.Log("Puzzle solved! Showing completed spell pattern...");
        
        // Wait for 2 seconds to let the user see the completed pattern
        yield return new WaitForSeconds(2f);
        
        // Now close the puzzle
        Debug.Log("Closing puzzle after showing completed pattern.");
        SolvePuzzle();
    }
    ...
    private void StopEnemySpawning()
    {
        ...
        Debug.Log($"CrackedWall2: Stopped enemy spawning. Total enemies spawned: {enemiesSpawned}");
    ...
    protected virtual void ChangeToNormalWall()
    {
        ...
        {
            Debug.LogWarning("CrackedWall2: No solved wall sprite assigned. Wall appearance will not change.");
            return;
        }
        ...
        if (spriteRenderer != null)
        {
            ...
            Debug.Log("CrackedWall2: Changed to normal wall sprite (SpriteRenderer).");
        }
        else if (uiImage != null)
        {
            uiImage.sprite = solvedWallSprite;
            Debug.Log("CrackedWall2: Changed to normal wall sprite (Image).");
        }
        else
        {
            Debug.LogError("CrackedWall2: No visual component found to change sprite!");
        }
        ...
        foreach (GameObject obj in allObjects)
        {
            ...
            if (IsSpellProjectile(obj))
            {
                ...
                if (rb != null && rb.simulated)
                {
                    ...
                    Debug.Log($"CrackedWall2: Frozen spell projectile: {obj.name}");
                }
            }
        }
        
        Debug.Log($"CrackedWall2: Frozen {frozenProjectiles.Count} spell projectiles");
    }
    ...
    private void FreezeEnemyBehavior()
    {
        ...
        Debug.Log($"CrackedWall2: Frozen {frozenEnemyAI.Count} enemy AI components and {frozenEnemyShooters.Count} enemy shooter components");
    }
    ...
    private void FreezeEnemyAIComponents(GameObject enemy)
    {
        ...
        foreach (string componentName in aiComponentNames)
        {
            ...
            if (aiComponent != null && aiComponent.enabled)
            {
                ...
                Debug.Log($"CrackedWall2: Frozen enemy AI component {componentName} on {enemy.name}");
            }
        }
    }
    ...
    private void FreezeEnemyShooterComponents(GameObject enemy)
    {
        ...
        foreach (string componentName in shooterComponentNames)
        {
            ...
            if (shooterComponent != null && shooterComponent.enabled)
            {
                ...
                Debug.Log($"CrackedWall2: Frozen enemy shooter component {componentName} on {enemy.name}");
            }
        }
    }
    ...
    private void UnfreezeSpellProjectiles()
    {
        ...
        for (int i = 0; i < frozenProjectiles.Count; i++)
        {
            if (frozenProjectiles[i] != null)
            {
                ...
                Debug.Log($"CrackedWall2: Unfrozen spell projectile: {frozenProjectiles[i].gameObject.name}");
            }
        }
        
        Debug.Log($"CrackedWall2: Unfrozen {frozenProjectiles.Count} spell projectiles");
    ...
    private void UnfreezeEnemyBehavior()
    {
        ...
        foreach (MonoBehaviour aiComponent in frozenEnemyAI)
        {
            if (aiComponent != null)
            {
                ...
                Debug.Log($"CrackedWall2: Unfrozen enemy AI component {aiComponent.GetType().Name} on {aiComponent.gameObject.name}");
            }
        }
        ...
        foreach (MonoBehaviour shooterComponent in frozenEnemyShooters)
        {
            if (shooterComponent != null)
            {
                ...
                Debug.Log($"CrackedWall2: Unfrozen enemy shooter component {shooterComponent.GetType().Name} on {shooterComponent.gameObject.name}");
            }
        }
        
        Debug.Log($"CrackedWall2: Unfrozen {frozenEnemyAI.Count} enemy AI components and {frozenEnemyShooters.Count} enemy shooter components");
    ...
    void OnDestroy()
    {
        ...
        // Clear all spell beams when destroyed
        if (spellGridController != null)
        {
            spellGridController.OnPuzzleClosed();
        }
    ...
    {
        Debug.Log("=== CrackedWall2 Reference Validation ===");
    ...
    {
        Debug.Log("=== CrackedWall2 Interaction Debug ===");
